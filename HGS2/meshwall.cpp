		...
		{//使っている透明でない壁
			...
			for (int nCntSnowBall = 0; nCntSnowBall < SNOWBALL_MAX; nCntSnowBall++, pSnowBall++)
			{
				if (pSnowBall->bUse)
				{
					...
					if ((Wallvec.z * Posvec.x) - (Wallvec.x * Posvec.z) <= 0.0f && (Wallvec.z * PosOldvec.x) - (Wallvec.x * PosOldvec.z) >= 0.0f && pos.y < g_aMeshWall[nCntWall].pos.y + MESHWALL_HEIGHT * g_aMeshWall[nCntWall].scale.y && pos.y > g_aMeshWall[nCntWall].pos.y)
					{//壁の外側
						WallCross = (Wallvec.z * movevec.x) - (Wallvec.x * movevec.z);
						PosCross = (Posvec.z * movevec.x) - (Posvec.x * movevec.z);
						PosCross /= WallCross;
