		...
		{//使っている透明でない壁
			...
			for (int nCntSnowBall = 0; nCntSnowBall < SNOWBALL_MAX; nCntSnowBall++, pSnowBall++)
			{
				if (pSnowBall->bUse)
				{
					...
					movevec = pos - posOld;//プレイヤーの移動ベクトル
					if ((Stagevec.x * Posvec.y) - (Stagevec.y * Posvec.x) <= 0.0f && (Stagevec.x * PosOldvec.y) - (Stagevec.y * PosOldvec.x) >= 0.0f && pos.z < g_aStage[nCntStage].pos.z + cosf(g_aStage[nCntStage].rot.y + D3DX_PI * 0.5f) * g_aStage[nCntStage].fLength && pos.z > g_aStage[nCntStage].pos.z + cosf(g_aStage[nCntStage].rot.y + D3DX_PI * -0.5f) * g_aStage[nCntStage].fLength)
					{//上
						StageCross = (Stagevec.x * movevec.y) - (Stagevec.y * movevec.x);
						PosCross = (Posvec.x * movevec.y) - (Posvec.y * movevec.x);
						PosCross /= StageCross;
